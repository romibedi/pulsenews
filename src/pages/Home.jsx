...
export default function Home() {
  ...
  const feedArticles = gridArticles.filter((a) => !clusteredIds.has(a.id));

  const pullquoteArticle = allArticles.slice(3, 12).find((a) => extractPullquote(a));
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className={`space-y-10 transition-opacity duration-300 ${isLangSwitching ? 'opacity-40' : ''}`}>
        ...
        {loading && feedArticles.length === 0 ? (
          ...
          breaks.push({ type: 'categories', key: 'categories' });

          if (sources.length > 0) {
            breaks.push({ type: 'sources', key: 'sources' });
          }

          // Local news section (if city detected and has articles)
          if (detectedCity && cityArticles.length > 0) {
            breaks.push({ type: 'local', key: 'local' });
          }
